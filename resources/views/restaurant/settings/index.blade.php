...
@section('content')
    <div class="container">
        ...
        <div id="settings_form">
            ...
            <div class="input-group" style="display: grid;">
                ...
                <select name="style" id="style" required disabled>
                    ...
                    <option value="Western" {{ $restaurant->restaurantStyle == 'Western' ? 'selected' : '' }}>Western
                    </option>
                    <option value="Fine Dining" {{ $restaurant->restaurantStyle == 'Fine Dining' ? 'selected' : '' }}>Fine
                        Dining</option>
                ...
                <div id="imagePreviewContainer" style="display: flex; gap: 10px; margin-top: 10px;">
                    ...
                    @foreach ($images as $index => $image)
                        <div class="image-wrapper">
                            ...
                            <input type="hidden" name="existing_images[{{ $index }}]"
                                value="{{ $image ?? 'null' }}">

                            <input type="file" name="image[{{ $index }}]" class="image-input"
                                data-index="{{ $index }}" accept="image/*" style="display: none;"
                                onchange="replaceImage(event, {{ $index }})" disabled>
                        </div>
                    @endforeach
                </div>
        ...
        <div id="editModal" class="modal">
            <div class="modal-content">
                ...
                <form id="settings_form" action="{{ route('restaurant.update', $restaurant->id) }}" method="POST"
                    ...
                    <div class="input-group" style="display: grid;">
                        ...
                        <select name="style" id="style" required>
                            <option value="Asian" {{ $restaurant->restaurantStyle == 'Asian' ? 'selected' : '' }}>Asian
                            </option>
                            <option value="Western" {{ $restaurant->restaurantStyle == 'Western' ? 'selected' : '' }}>
                                Western</option>
                            <option value="Fine Dining"
                                {{ $restaurant->restaurantStyle == 'Fine Dining' ? 'selected' : '' }}>Fine Dining</option>
                            <option value="Bar" {{ $restaurant->restaurantStyle == 'Bar' ? 'selected' : '' }}>Bar
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="input-group" style="display: grid; gap:20px">
                        ...
                        <button type="button" id="add-schedule"
                            style="width: min-content; background:transparent; color:#4286f5; text-decoration:underline; white-space:nowrap">Add</button>
                    </div>
                        ...
                        <div id="imagePreviewContainer" style="display: flex; gap: 10px; margin-top: 10px;">
                            ...
                            @foreach ($images as $index => $image)
                                <div class="image-wrapper">
                                    ...
                                    <input type="file" name="image[{{ $index }}]" class="image-input"
                                        data-index="{{ $index }}" accept="image/*" style="display: none;"
                                        onchange="replaceImage(event, {{ $index }})">
                                </div>
                            @endforeach
                        </div>
        ...
        function replaceImage(event, index) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    document.querySelector("#editModal").querySelector(`.preview-image[data-index="${index}"]`).src = e
                        .target.result;
                };
    ...
    <script>
        ...
        document.getElementById("add-schedule").addEventListener("click", function() {
            ...
            const selectedDays = [...document.querySelector("#editModal").querySelectorAll(".days-dropdown")].map(
                select => select.value);
